...
export interface MosServerSummaryCardConfig extends LovelaceCardConfig {
  ...
  /** Shared tap/hold/double-tap actions applied to every pool pill. Support [[pool_name]]/[[pool_device_id]]/[[pool_usage_entity]]/[[pool_problem_entity]] tokens, substituted per pool at fire time. [[pool_device_id]] is what a fire-dom-event popup-card action would pass as mos-detail-card's device_id. */
  ...
  /**
   * Tap/hold/double-tap actions for the CPU temperature pill. Support
   * [[server_name]]/[[server_device_id]]/[[cpu_temp_entity]] tokens.
   *
   * `tap_action` is a fallback, not an override: when left unset, tapping
   * the pill instead cycles its displayed reading through Main/Average/Max.
   * Setting it restores exactly today's behavior for that gesture.
   * `hold_action`/`double_tap_action` have no built-in fallback of their
   * own — a typical use for `hold_action` is opening the
   * [mos-detail-card](../mos-detail-card) package's `"server"` kind via
   * `fire-dom-event`/popup-card with `device_id: "[[server_device_id]]"`,
   * which shows all three readings together in one of several layouts (see

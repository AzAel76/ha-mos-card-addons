...
interface PoolInfo {
  name: string;
  /** The pool's own device_id — lets `pool_tap_action` open a `mos-detail-card` popup for this exact pool via `device_id: "[[pool_device_id]]"`. */
  deviceId: string;
  usageEntity?: string;
...
interface Resolved {
  ...
  cpuTemperatureAverageEntity?: string;
  cpuTemperatureMaxEntity?: string;
...
export class MosServerSummaryCard extends LitElement {
  ...
  /** Which of Main/Average/Max the CPU Temp pill currently shows — advanced by tap when `cpu_temp_tap_action` isn't configured. Index into `_cpuTempReadings()`'s result, wrapped modulo its (possibly-changing, if an older server lacks average/max) length at read time rather than stored as an id, so a transient resolution gap can't leave it pointing past the end. */
  @state() private _cpuTempCycleIndex = 0;
  ...
  private _resolve(): Resolved | undefined {
    ...
    return {
      ...
      cpuTemperatureAverageEntity: metric(CPU_TEMPERATURE_AVERAGE),
      cpuTemperatureMaxEntity: metric(CPU_TEMPERATURE_MAX),
  ...
  /**
   * Fetches CPU-load/memory-usage (and, when the CPU-temperature detail
   * dialog's layout needs it, CPU-temperature) history once per
   * (entities, window) combination — not on every render — then hands off
   * to HistoryBuffer, which extends the window locally from live ticks
   * afterward (see history.ts). Fire-and-forget: `requestUpdate()` on
   * completion is what actually shows the fetched trend line, since
  ...
  /**
   * The Main/Average/Max readings that actually resolved, in that fixed
   * order — an older MOS/ha-mos server may lack Average/Max entirely, so
   * this is never assumed to be exactly 3 long. Single source of truth for
   * both the pill's currently-cycled reading and every detail-dialog layout.
   */
  private _cpuTempReadings(resolved: Resolved): { id: "main" | "average" | "max"; label: string; entity: string }[] {
    const readings: { id: "main" | "average" | "max"; label: string; entity: string }[] = [];
    if (resolved.cpuTemperatureEntity) {
      readings.push({ id: "main", label: "Main", entity: resolved.cpuTemperatureEntity });
    }
    if (resolved.cpuTemperatureAverageEntity) {
      readings.push({ id: "average", label: "Average", entity: resolved.cpuTemperatureAverageEntity });
    }
    if (resolved.cpuTemperatureMaxEntity) {
      readings.push({ id: "max", label: "Max", entity: resolved.cpuTemperatureMaxEntity });
    }
    return readings;
  }
  ...
  /**
   * Tap falls back to a built-in behavior (cycling the pill's reading)
   * whenever `cpu_temp_tap_action` isn't configured — a user who already
   * set a custom action keeps exactly that, unchanged. Hold has no
   * built-in fallback (same as `pool_hold_action`): the multi-reading
   * detail view lives in the separate `mos-detail-card` package now,
   * opened explicitly via `fire-dom-event`/popup-card using the
   * `[[server_device_id]]` token below — see this package's README.
   */
  private _fireCpuTempAction(resolved: Resolved, action: GestureAction): void {
    ...
    if (!actionConfig) {
      if (action === "tap") {
        const count = this._cpuTempReadings(resolved).length;
        if (count > 1) {
          this._cpuTempCycleIndex = (this._cpuTempCycleIndex + 1) % count;
        }
      }
      return;
    }
    const values = {
      ...
      server_device_id: this._config.server,
  ...
  private _renderCard(resolved: Resolved) {
    ...
    const cpuTempReadings = this._cpuTempReadings(resolved);
    const cpuTempCurrent = cpuTempReadings.length
      ? cpuTempReadings[this._cpuTempCycleIndex % cpuTempReadings.length]
      : undefined;
    const cpuTemperature = cpuTempCurrent ? Number(hass.states[cpuTempCurrent.entity]?.state) : undefined;
              ...
              ${
                showCpuTemp
                  ? html`
                      <div
                        class="pill"
                        @pointerdown=${cpuTempHandlers.onPointerDown}
                        @pointerup=${cpuTempHandlers.onPointerUp}
                        @pointercancel=${cpuTempHandlers.onPointerCancel}
                      >
                        <div class="pill-gauge">
                          <mos-server-gauge .value=${cpuTemperature} icon="mdi:thermometer"></mos-server-gauge>
                        </div>
                        <div class="pill-text">
                          <div class="pill-value">
                            ${
                          ...
                          <div class="pill-label">
                            CPU
                            Temp${cpuTempCurrent && cpuTempCurrent.id !== "main" ? ` · ${cpuTempCurrent.label}` : ""}
                          </div>
                        </div>
                      </div>
                    `

...
DEST_DIR="$HA_CONFIG_PATH/www/community/mos-server-summary-card"

npm run build

echo "Deploying dist/mos-server-summary-card.js to $HA_HOST:$DEST_DIR"
ssh "$HA_HOST" "mkdir -p '$DEST_DIR'"
scp dist/mos-server-summary-card.js "$HA_HOST:$DEST_DIR/mos-server-summary-card.js"

echo "Done. Add/refresh the Lovelace resource:"
echo "  URL:  /local/community/mos-server-summary-card/mos-server-summary-card.js"

 ...
 *
 * Named `mos-server-gauge` (not `mos-memory-gauge`, matching
 * mos-kind-title-card's identical component) since both cards can be
 * installed together on one dashboard — `customElements.define()` throws on
 * a duplicate tag name, and each package here is a fully standalone bundle
 * with no shared module to dedupe the definition through.
 */
...
export class MosServerGauge extends LitElement {
